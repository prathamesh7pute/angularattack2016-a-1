...
export class ExpenseService {
  ...
  createExpense(name: string, category: string, description: string, createdAt: number):
      Promise<any> {
    return this.expenseItems$.push(new Expense(name, category, description, createdAt));
  }

  removeExpense(expense: IExpense): Promise<any> { return this.expenseItems$.remove(expense.$key); }
